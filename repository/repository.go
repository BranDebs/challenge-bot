...
type User interface {
	...
	ListUsers(ctx context.Context, filter Filters, offset, limit uint) ([]*model.User, error)
}

type Goal interface {
	CreateGoal(ctx context.Context, goal *model.Goal) error
	FindGoal(ctx context.Context, id uint64) (*model.Goal, error)
	ListGoals(ctx context.Context, filter Filters, offset, limit uint) ([]*model.Goal, error)
}
...
type Progress interface {
	...
	ListProgress(ctx context.Context, filter Filters, offset, limit uint) ([]*model.Progress, error)
}
